...
for index, row in df.iterrows():
    ...
    for lyric_line in processed_lyrics:
        all_processed_lyrics.append({
            'Song Title': song_title,
            'Artist': artist,
            'Cleaned Lyrics': lyric_line
        })

# Create a new DataFrame with each processed line associated with its song title and artist
new_df = pd.DataFrame(all_processed_lyrics)

# Save the new DataFrame to a CSV
output_path = 'output_file2.csv'
